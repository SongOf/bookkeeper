 ...
 * Test cookies.
 */
public class CookieIndexTest extends BookKeeperClusterTestCase {
    final int bookiePort = PortManager.nextFreePort();

    public CookieIndexTest() {
        super(0);
    }
    ...
    public void testDirectoryMissing() throws Exception {
        ...
        conf.setJournalDirName(journalDir).setLedgerDirNames(ledgerDirs).
                setIndexDirName(new String[] { indexDirs[0], indexDirs[1]});
        try {
            Bookie b2 = new Bookie(conf);
            fail("Shouldn't have been able to start");
        } catch (InvalidCookieException ice) {
    ...
    public void testCookieMissingOnIndexDir() throws Exception {
        ...
        File cookieFile =
                new File(Bookie.getCurrentDirectory(new File(indexDirs[0])), BookKeeperConstants.VERSION_FILENAME);
        assertTrue(cookieFile.delete());
        try {
            new Bookie(conf);
            fail("Shouldn't have been able to start");
