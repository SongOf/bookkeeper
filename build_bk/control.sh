...
function init_server_info() {
    ...
    if [ -f "${BOOKIE_HOME}/.deploy/service.service_name.txt" ]; then
     ...
     if [[ $SERVICE_NAME == *"bookkeeper"*"iot"* ]]; then #处理iot场景的bookkeeper集群
     ...
     elif [[ ${SERVICE_CLUSTER_NAME} == hnb-pre-v.preview-air.preview-bookkeeper.dop.ddmq.didi.com ]]; then
         if [[ ${is_check_ssd} == "true" ]];then
             check_ssd 'data0'
         fi
     ...
     elif [[ ${SERVICE_CLUSTER_NAME} == pus01-pre-v.preview.preview-bookkeeper.dop.ddmq.didi.com ]]; then
         if [[ ${is_check_ssd} == "true" ]];then
             check_ssd 'data1'
         fi
     ...
     elif [[ ${SERVICE_CLUSTER_NAME} == hnd-v.level1-pro.bookkeeper.dop.ddmq.didi.com ]]; then
         if [[ ${is_check_ssd} == "true" ]];then
             check_ssd 'data0'
             check_ssd 'data1'
             check_ssd 'data5'
         ...
         BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.map.conf"
     elif [[ ${SERVICE_CLUSTER_NAME} == hnb-v.map-air.bookkeeper.dop.ddmq.didi.com ]]; then
         if [[ ${is_check_ssd} == "true" ]];then
             check_ssd 'data0'
         fi
     ...
     elif [[ ${SERVICE_CLUSTER_NAME} == hnb-v.level3-air.bookkeeper.dop.ddmq.didi.com ]]; then
              if [[ ${is_check_ssd} == "true" ]];then
                  check_ssd 'data0'
              fi
              BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.level3-air.ys.conf"
     elif [[ ${SERVICE_CLUSTER_NAME} == hna-v.level3-air.bookkeeper.dop.ddmq.didi.com ]]; then
              if [[ ${is_check_ssd} == "true" ]];then
                  check_ssd 'data0'
              fi
              BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.level3-air.gz01.conf"
     elif [[ ${SERVICE_CLUSTER_NAME} == hnb-v.level2-air.bookkeeper.dop.ddmq.didi.com ]]; then
              if [[ ${is_check_ssd} == "true" ]];then
                  check_ssd 'data0'
              fi
              BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.level2-air.ys.conf"
     elif [[ ${SERVICE_CLUSTER_NAME} == hna-v.level2-air.bookkeeper.dop.ddmq.didi.com ]]; then
              if [[ ${is_check_ssd} == "true" ]];then
                  check_ssd 'data0'
              fi
     ...
     elif [[ ${SERVICE_CLUSTER_NAME} == hna-v.level1-air.bookkeeper.dop.ddmq.didi.com ]]; then
              if [[ ${is_check_ssd} == "true" ]];then
                  check_ssd 'data0'
              fi
              BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.level1-air.gz01.conf"
     elif [[ ${SERVICE_CLUSTER_NAME} == hnc-v.level1-air.bookkeeper.dop.ddmq.didi.com ]]; then
              if [[ ${is_check_ssd} == "true" ]];then
                  check_ssd 'data0'
              fi
              BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.level1-air.hnc.conf"
     elif [[ ${SERVICE_CLUSTER_NAME} == hnf-v.level1-air.bookkeeper.dop.ddmq.didi.com ]]; then
              if [[ ${is_check_ssd} == "true" ]];then
                  check_ssd 'data0'
              fi
        ...
        BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.level3.gz01.conf"
         ...
         case $HOSTNAME in
     ...
     elif [[ ${SERVICE_CLUSTER_NAME} == pus01-v.level3-air.bookkeeper.dop.ddmq.didi.com ]]; then
          if [[ ${is_check_ssd} == "true" ]];then
              check_ssd 'data0'
          fi
          BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.level3-air.pus01.conf"
     elif [[ ${SERVICE_CLUSTER_NAME} == pus01-v.level3.bookkeeper.dop.ddmq.didi.com ]]; then
          if [[ ${is_check_ssd} == "true" ]];then
              check_ssd 'data0'
          fi
          BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.level3.pus01.conf"
     elif [[ ${SERVICE_CLUSTER_NAME} == pus02-v.level3.bookkeeper.dop.ddmq.didi.com ]]; then
          if [[ ${is_check_ssd} == "true" ]];then
              check_ssd 'data0'
          fi
          BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.level3.pus02.conf"
     elif [[ ${SERVICE_CLUSTER_NAME} == pus02-v.level3-air.bookkeeper.dop.ddmq.didi.com ]]; then
          if [[ ${is_check_ssd} == "true" ]];then
              check_ssd 'data0'
          fi
          BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.level3-air.pus02.conf"
      elif [[ ${SERVICE_CLUSTER_NAME} == pus03-v.level3-pro.bookkeeper.dop.ddmq.didi.com ]]; then
           if [[ ${is_check_ssd} == "true" ]];then
               check_ssd 'data0'
           fi
           BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.level3-pro.pus03.conf"
      elif [[ ${SERVICE_CLUSTER_NAME} == pus06-v.level3-pro.bookkeeper.dop.ddmq.didi.com ]]; then
           if [[ ${is_check_ssd} == "true" ]];then
               check_ssd 'data0'
           fi
           BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.level3-pro.pus06.conf"
     ...
     elif [[ ${SERVICE_CLUSTER_NAME} == hne-v.level1-air.bookkeeper.kflower.dop.ddmq.didi.com ]]; then
          if [[ ${is_check_ssd} == "true" ]];then
              check_ssd 'data0'
          fi
          ...
          BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.level1.hbf.conf"
     elif [[ ${SERVICE_CLUSTER_NAME} == hbf-v.perf.bookkeeper.dop.ddmq.didi.com ]]; then
          if [[ ${is_check_ssd} == "true" ]];then
              check_ssd 'data0'
          fi
          BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.perf.hbf.conf"
     elif [[ ${SERVICE_CLUSTER_NAME} == us01-v.level2.bookkeeper.dop.ddmq.didi.com ]]; then
          if [[ ${is_check_ssd} == "true" ]];then
              check_ssd 'data0'
          fi
          BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.level2.us01.conf"
     elif [[ ${SERVICE_CLUSTER_NAME} == us01-v.level2-air.bookkeeper.dop.ddmq.didi.com ]]; then
          if [[ ${is_check_ssd} == "true" ]];then
              check_ssd 'data0'
          fi
          BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.level2-air.us01.conf"
     elif [[ ${SERVICE_CLUSTER_NAME} == xna-v.level2.bookkeeper.dop.ddmq.didi.com ]]; then
          if [[ ${is_check_ssd} == "true" ]];then
              check_ssd 'data0'
          fi
          BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.level2.styd.conf"
     elif [[ ${SERVICE_CLUSTER_NAME} == xna-v.level2-air.bookkeeper.dop.ddmq.didi.com ]]; then
          if [[ ${is_check_ssd} == "true" ]];then
              check_ssd 'data0'
          fi
          BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.level2-air.styd.conf"
     elif [[ ${SERVICE_CLUSTER_NAME} == hbd-hbdsim-v.bookkeeper.rt.native.ddmq.didi.com ]]; then
          if [[ ${is_check_ssd} == "true" ]];then
              check_ssd 'data0'----------
          fi
          BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.native.rt.hbd.sim.conf"
     elif [[ ${SERVICE_CLUSTER_NAME} == hna-v.level2-pro.bookkeeper.dop.ddmq.didi.com ]]; then
        if [[ ${is_check_ssd} == "true" ]];then
            check_ssd 'data0'
        fi
        ...
        BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.bike-pro.hnc.conf"
          ...
          BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.native.rt-public.hna.conf"
     ...
     elif [[ ${SERVICE_CLUSTER_NAME} == hnd-v.rt-public.bookkeeper.native.ddmq.didi.com ]]; then
          if [[ ${is_check_ssd} == "true" ]];then
              check_ssd 'data0'----------
          fi
          BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.native.rt-public.hnd.conf"
     elif [[ ${SERVICE_CLUSTER_NAME} == hnd-v.rt-core.bookkeeper.native.ddmq.didi.com ]]; then
          if [[ ${is_check_ssd} == "true" ]];then
              check_ssd 'data0'----------
          fi
          BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.native.rt-core.hnd.conf"
     elif [[ ${SERVICE_CLUSTER_NAME} == pus05-v.rt-core.bookkeeper.native.ddmq.didi.com ]]; then
          if [[ ${is_check_ssd} == "true" ]];then
              check_ssd 'data0'----------
          fi
          BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.native.rt-core.pus05.conf"
     elif [[ ${SERVICE_CLUSTER_NAME} == hna-pre-v.perf.bookkeeper.native.ddmq.didi.com ]]; then
          if [[ ${is_check_ssd} == "true" ]];then
              check_ssd 'data0'----------
          fi
     ...
     elif [[ ${SERVICE_CLUSTER_NAME} == us01-v.level2-S2103-pro.bookkeeper.dop.ddmq.didi.com ]]; then
         if [[ ${is_check_ssd} == "true" ]];then
             check_ssd 'data0'
             check_ssd 'data1'
             check_ssd 'data5'
        ...
        BOOKIE_CONF="${BOOKIE_HOME}/conf/bk_conf/bookkeeper.level2-S2103-pro.us01.conf"
     elif [[ ${SERVICE_CLUSTER_NAME} == hnb-v.level2-S2103-pro.bookkeeper.dop.ddmq.didi.com ]]; then
         if [[ ${is_check_ssd} == "true" ]];then
             check_ssd 'data0'
             check_ssd 'data1'
             check_ssd 'data5'
